---
title: "A grammer for data type conversion"
...
* If there is any errors you know where in the script it should be fixed
* It scales up your code. For example, all columns that should be numeric could be converted at the same time.

Additionally, if every columns are explicitly converted to its appropriate data type then you won't be suprised by data type errors the next time you run the script. 
...
### Support functions
`convert` must be used in conjuction with data type conversion functions:
...
### The syntax
Imagine you have a data frame where cou want to change columns:
...
Which gives thre same result. However, you need to refer to the column name twice and the data type conversion function for each column. Imagine the code to convert 20 columns. 

However, `dplyr` have a another way to help with applying the same function to multiple columns which could help, `mutate_at`. The same example could then be:
...
Which is very scalable for adding more variables for data type conversion. However, it takes much more code to produce the same result. In fact, `convert` uses `mutate_at` internally. The difference is syntax and code readability. Compare again with `convert`:
...
All support functions of `convert` support additional arguments to be passed. For example, if you want to convert a number to a date and want to include an `origin` argument you can write:
```{r}
